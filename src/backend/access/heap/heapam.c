...
initscan(HeapScanDesc scan, ScanKey key, bool is_rescan)
{
	...
	{
		scan->rs_buf[i] = InvalidBuffer;
	}

	scan->rs_tbmres=MAX_LIBAIO;
...
heap_getnext(HeapScanDesc scan, ScanDirection direction)
{
	...
	/* If scan->rs_snapshot is an MVCC-safe snapshot */
	if (scan->rs_pageatatime) 
...
#if DASOM_LOG 
		/*DASOM - 280715_log*/
		fprintf(stderr, "2nd tid=(%u,%u)\n", ItemPointerGetBlockNumber(tid), offnum);
#endif
		return true;
	}
...
//	ItemPointer tid = &(tuple->t_self); //array?
	...
	for(i = 0; i < mbufNum; i++)
	{
		...
		//DASOM
		blk = ItemPointerGetBlockNumber(&tidList[i]);
	   ...
	   /*
		...
		if (offnum < FirstOffsetNumber || offnum > PageGetMaxOffsetNumber(page[i]))
		{
			...
			if(!keep_buf)
			{
				fprintf(stderr,"%dth out-of-range release\n", i); /*DASOM log*/
				
				/*DASOM All  pinned buffers should be released. */
				for(j = i; j >= 0; j--) {  
					ReleaseBuffer(m_buffer[j]);
					m_buffer[j] = InvalidBuffer;
				}		
			}
			tuple[i].t_data = NULL;
			return false;
		...
		if (!ItemIdIsNormal(lp[i]))
		{
			...
			if (!keep_buf)
			{
				fprintf(stderr,"deleted tuple released\n"); /*DASOM log*/
		 ...
		 * check time qualification of tuple, then release lock
		 */
//		valid = HeapTupleSatisfiesVisibility(&tuple[i], snapshot, m_buffer[i]);
...
//		fprintf(stderr,"%dth tuple : buffer-%d\n",i,m_buffer[i]); /*DASOM log*/
...
#if 1
		/*DASOM : print out invalid buffer info */
		else {
			fprintf(stderr, "%d th buffer id : %d, fails to pass the validation\n", i, m_buffer[i]);
			test_valid=false;
		}

		CheckForSerializableConflictOut(valid[i], relation, &tuple[i], m_buffer[i], snapshot);
#else
		CheckForSerializableConflictOut(valid, relation, &tuple[i], m_buffer[i], snapshot);
#endif
		LockBuffer(m_buffer[i], BUFFER_LOCK_UNLOCK);
	}
	...
	/*We couldn't pass the validation test so all pinned buffers should be released.*/
	if(!keep_buf){
		fprintf(stderr, "read buffer fail : end of heap-fetch-multiple release\n"); /*DASOM log*/
		for (i = 0; i < mbufNum ; i++)
...
bool
heap_fetch_multiple(Relation relation, 							//IN scan->heapRelation
		   Snapshot snapshot,									//IN scan->xs_snapshot
		   HeapTuple tuple, //caller sends HeapTupleData array 	//IN/OUT
		   bool keep_buf, // fixed value; FALSE 				//IN
		   Relation stats_relation, 							//IN
		   Buffer *m_buffer, 									//IN/OUT
		   int mbufNum,											//IN
		   ItemPointerData *tidList)							//IN
{
//	ItemPointer tid = &(tuple->t_self); //array?
	ItemId		lp[mbufNum];
	Page		page[mbufNum]; // array
	OffsetNumber offnum;
	//bool		valid;
	bool		valid[mbufNum]; //the same with got_heap_tuple in "index_fetch_heap" in "indexam.c"
	bool		test_valid = true;
	int 		i;
	...
	for(i = 0; i < mbufNum; i++)
	{
		...
		LockBuffer(m_buffer[i], BUFFER_LOCK_SHARE);
		valid[i] = heap_hot_search_buffer(&tidList[i], relation,
										  m_buffer[i], snapshot, 
										  &tuple[i], &all_dead,
										  true); /*!scan->xs_continue_hot*/
		LockBuffer(m_buffer[i], BUFFER_LOCK_UNLOCK);
#if DASOM_LOG
		blk = ItemPointerGetBlockNumber(&tidList[i]);
		offnum = ItemPointerGetOffsetNumber(&tidList[i]);
		...
		{
#if DASOM_LOG
			fprintf(stderr, "%d-th valid\n",i);
//#endif
			if(stats_relation !=NULL) //Should check it but, in my case, never "NULL".
			{
				pgstat_count_heap_fetch(stats_relation);
			}
#endif
		...
		{
			ReleaseBuffer(m_buffer[i]);
			ItemPointerSetInvalid(&tidList[i]);
#if DASOM_LOG
			fprintf(stderr, "%d-th invalid\n",i);
#endif
		//	test_valid = false;
		}
	}
	return true;
#if 0
//should be modified here as well	
	if (test_valid)
		return true;
#endif
	/*We couldn't pass the validation test so all pinned buffers should be released.*/
