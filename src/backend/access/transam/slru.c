			  ...
			  LWLockId ctllock, const char *subdir)
{
	...
	if (!IsUnderPostmaster)
	{
		...
		/*
		 * FIXME:DASOM referencing WOONHAK 
		 * add o_direct flags when opening file.
		 * compile with define DIRECT_IO to make it active
		 */
#ifdef DIRECT_IO
...
SlruPhysicalReadPage(SlruCtl ctl, int pageno, int slotno)
{
	 ...
	 * FIXME:DASOM referencing WOONHAK
	 * add o_direct flags when opening file.
	 * compile with defind DIRECT_IO to make it active.
	 */
#ifdef DIRTECT_IO
...
SlruPhysicalWritePage(SlruCtl ctl, int pageno, int slotno, SlruFlush fdata)
{
	...
	if (fd < 0)
	{
		 ...
		 * FIXME:DASOM referencing WOONHAK
		 * add o_direct flags when opening file.
		 * compile with define DIRECT_IO to make this flags active.
		 */
#ifdef DIRTECT_IO
		fd = OpenTransientFile(path, O_RDWR | O_CREAT | PG_BINARY,
			   S_IRUSR | S_IWUSR | O_DIRECT);
#else
		fd = OpenTransientFile(path, O_RDWR | O_CREAT | PG_BINARY,
			   S_IRUSR | S_IWUSR);
#endif
