...
static TupleTableSlot *IndexNext_Multiple(IndexScanState *node);
static TupleTableSlot *___IndexNext(IndexScanState *node);
static int itemptr_comparator2(const void *a, const void *b);
...
#if 0
static TupleTableSlot *
__IndexNext(IndexScanState *node)
{
	EState	   *estate;
	ExprContext *econtext;
	ScanDirection direction;
	...
	ItemPointer tid;
//	static ItemPointer _tidListptr = NULL; //DASOM -- unused
	int *numTids;
	long numberTuples = MAX_LIBAIO; //Inital value to allocate tid-array
	BlockNumber blkNum;
	unsigned int relid;
	Relation rel;

//	_tidList = (ItemPointerData *)palloc(MAXTID*sizeof(ItemPointerData));
	...
	{
		//numberTuples = estate->numberTuples;
		estate->_tidListptr = (ItemPointerData *) 
								palloc(numberTuples * sizeof(ItemPointerData));
		//_tidListptr = (ItemPointerData *)
		//						palloc(numberTuples * sizeof(ItemPointerData));
	}
	/*if(_tidListptr ==NULL)
	{
		_tidListptr = estate->_tidListptr;
	}*/

	//_tidListptr = estate->_tidListptr; // for dynamic allocation.
	//_tidListptr = estate->_tidList; // for static allocation.
	...
	while ((tid = __index_getnext(scandesc, direction)) != NULL)
	{
		/*
		 * FIXME:DASOM
		 * Store the Tids in the list
		
		
		_tidList[numTids++]=*tid;
		*/
		...
		/* FIXME:DASOM*/
		if((*numTids) >= numberTuples)
		{
			numberTuples = numberTuples * 2;
			estate->_tidListptr = (ItemPointerData *)
				repalloc(estate->_tidListptr, numberTuples * sizeof(ItemPointerData));
		}

//		estate->_tidListptr[(*numTids)++]=*tid;
		estate->_tidListptr[(*numTids)]=*tid;
		(*numTids)++;
...
/*
 * FIXME:DASOM - 200815
 * Copy itemptr_comparator from nodeTidscan.c
 * qsort comparator for ItemPointerData items
 */
static int
itemptr_comparator2(const void *a, const void *b)
{
...
#if 1 //Used
static TupleTableSlot *
IndexNext_Multiple(IndexScanState *node)
{
	EState	   *estate;
	ExprContext *econtext;
	ScanDirection direction;
	...
	/* 
	 * DASOM 8th Oct, 2015
	 * Add for tidList-per-plan
	 */
	 IndexScan *myPlan;
	 myPlan = (IndexScan *) node->ss.ps.plan;
	 ...
	 * FIXME:DASOM
	 */
	tuple = myPlan->iss_htup;
	buffer = myPlan->xs_mbuf;
	
	if(myPlan->_tidListptr == NULL)
	{		
		myPlan->_tidListptr = (ItemPointerData *) 
								palloc(numberTuples * sizeof(ItemPointerData));
	
		numTids = &(myPlan->numtid);
#if DASOM_LOG
		fprintf(stderr, "LOG: Allocate LIST\n");
#endif
		/*
		 * ok, now that we have what we need, fetch the next tuple.
		 */
		if (myPlan->_tidListptr != NULL)
		{
			while ((tid = __index_getnext(scandesc, direction)) != NULL)
			{
				...
				if((*numTids) >= numberTuples)
				{
					numberTuples = numberTuples * 2;
					myPlan->_tidListptr = (ItemPointerData *)
						repalloc(myPlan->_tidListptr, numberTuples * sizeof(ItemPointerData));
				}

				myPlan->_tidListptr[(*numTids)]=*tid;
				(*numTids)++;
...
#if DASOM_LOG
			fprintf(stderr, "LOG:  Create List, %d\n", *numTids);
#endif
			if((*numTids)>1)
			{
				int lastTid;
				int i;
				/* Do what to do */
				tidList = myPlan->_tidListptr;
				/* Phase 1. Sorting */
				qsort((void *) tidList, *numTids, sizeof(ItemPointerData), itemptr_comparator2);
...
#if DASOM_LOG		
		fprintf(stderr, "LOG: Sort List, %d\n", *numTids);
#endif
//		tuple=node->iss_htup;
		myPlan->iss_remain_tid=*numTids;
		myPlan->fidx_state=FetchTuples;
	}

	while (myPlan->iss_remain_tid !=0)
	{
		tidList = myPlan->_tidListptr;
		switch((myPlan->fidx_state))
		{
			case FetchTuples:
				/* Phase 3. Read tuples */
				if(myPlan->iss_remain_tid < MAX_LIBAIO)
				{
					myPlan->iss_mbufNum = myPlan->iss_remain_tid;
				}
/*Instead of NULL for stats_relation, Pass scandesc->indexRelation*/
				if(heap_fetch_multiple(scandesc->heapRelation, scandesc->xs_snapshot, 
										tuple, false, scandesc->indexRelation, buffer, myPlan->iss_mbufNum, 
										&tidList[myPlan->iss_TidPtr]))
				{
#if DASOM_LOG
					fprintf(stderr, "LOG: Fetch tuples, %d\n", myPlan->iss_mbufNum);
#endif
					myPlan->fidx_state=ReturnTuples;
				}
				else 
					break;

			case ReturnTuples:
				/* Phase 4. Return Slot containing tuple */
				while(myPlan->cur_tuple < myPlan->iss_mbufNum)
				{
					if(ItemPointerIsValid(&tidList[(myPlan->iss_TidPtr)+(myPlan->cur_tuple)]))
					{
						ExecStoreTuple(&tuple[myPlan->cur_tuple],
									  slot,
									  buffer[myPlan->cur_tuple],
									  false);
							
						ReleaseBuffer(buffer[myPlan->cur_tuple]);
#if DASOM_LOG
						fprintf(stderr, "LOG: Return %d-th slot.\n", myPlan->cur_tuple);	
#endif
						myPlan->cur_tuple++;
						return slot;
					}
...
#if DASOM_LOG		
				fprintf(stderr, "LOG: Finish return tuples.\n");
#endif
				myPlan->cur_tuple=0;
				myPlan->iss_TidPtr+=myPlan->iss_mbufNum;
				myPlan->iss_remain_tid-=myPlan->iss_mbufNum;
				myPlan->fidx_state=FetchTuples;
...
#endif
//		tuple=node->iss_htup;
		node->iss_remain_tid=*numTids;
		node->fidx_state=FetchTuples;
	}

	while (node->iss_remain_tid !=0)
	{
		tidList = myPlan->_tidListptr;
		switch((node->fidx_state))
		{
			case FetchTuples:
				/* Phase 3. Read tuples */
				if(node->iss_remain_tid < MAX_LIBAIO)
				{
					node->iss_mbufNum = node->iss_remain_tid;
				}
/*Instead of NULL for stats_relation, Pass scandesc->indexRelation*/
				if(heap_fetch_multiple(scandesc->heapRelation, scandesc->xs_snapshot, 
										tuple, false, scandesc->indexRelation, buffer, node->iss_mbufNum, 
										&tidList[node->iss_TidPtr]))
				{
#if DASOM_LOG
					fprintf(stderr, "LOG: Fetch tuples, %d\n", node->iss_mbufNum);
#endif
					node->fidx_state=ReturnTuples;
				}
				else 
					break;
			...
			case ReturnTuples:
				/* Phase 4. Return Slot containing tuple */
				while(node->cur_tuple < node->iss_mbufNum)
				{
					if(ItemPointerIsValid(&tidList[(node->iss_TidPtr)+(node->cur_tuple)]))
					{
						ExecStoreTuple(&tuple[node->cur_tuple],
									  slot,
									  buffer[node->cur_tuple],
									  false);
...
#endif
				node->cur_tuple=0;
				node->iss_TidPtr+=node->iss_mbufNum;
				node->iss_remain_tid-=node->iss_mbufNum;
				node->fidx_state=FetchTuples;
...
IndexNext_Multiple(IndexScanState *node)
{
	...
	FetchTuples:
		/* Phase 3. Read tuples */
		if(remain_tid < MAX_LIBAIO)
		{
			node->iss_mbufNum = remain_tid;
		}

		if(heap_fetch_multiple(scandesc->heapReleation, scandesc->xs_snapshot, tuple, 
									false, NULL, buffer, node->iss_mbufNum, &tidList[node->iss_TidPtr]))
		{
			node->fidx_state=ReturnTuples;
		}
		else 
	...
	case ReturnTuples:
		...
		while(node->cur_tuple < node->iss_mbufNum)
		{
			if(ItemPointerIsValid(&tidList[(node->iss_TidPtr)+(node->cur_tuple)]))
			{
				ExecStoreSlot(&tuple[node->cur_tuple],
							  slot,
							  buffer[node->cur_tuple],
							  false);
...
ExecIndexScan(IndexScanState *node)
{
	...
	/*
	 * if TidScan
	 * modify using "outernode"-related function
	 * nodeTag(outerPlanState(node))
	 */ 
//	if((node->ss.ps.parentType)==T_TidScanState)
//		return ExecScan(&node->ss,
//						(ExecScanAccessMtd) __IndexNext,//IndexNext,   /*FIXME:DASOM IndexNext->__IndexNext*/
//						(ExecScanRecheckMtd) IndexRecheck);
#if 1		
	/*else*/
	return ExecScan(&node->ss,
					(ExecScanAccessMtd) IndexNext_Multiple,
					(ExecScanRecheckMtd) IndexRecheck);
#endif
}
...
#if 0 // They have been moved to IndexScan structure, 16th Oct 2015

	/* DASOM - Init new vars */
	indexstate->cur_tuple=0;
	indexstate->iss_TidPtr=0;
	indexstate->fidx_state = CreateList;
	indexstate->iss_mbufNum=MAX_LIBAIO;
	indexstate->iss_remain_tid=0;
#endif 
#if 0
	/* DASOM - Init new vars */
	indexstate->ss.ps.cur_tuple=0;
	indexstate->ss.ps.iss_TidPtr=0;
	indexstate->ss.ps.fidx_state = CreateList;
	indexstate->ss.ps.iss_mbufNum=MAX_LIBAIO;
	indexstate->ss.ps.iss_remain_tid=0;
	indexstate->ss.ps._tidListptr=NULL;
	indexstate->ss.ps.numtid=0;
#endif
