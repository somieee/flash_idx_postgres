			   ...
			   ScanDirection indexscandir)
{
	...
	/*
	 * DASOM : 8th Oct, 2015
	 * Put tidListptr, cursor and etc here as expecting their status will be permanent during the excution.
	 */
	node->numtid=0;
	node->_tidListptr=NULL;
	node->cur_tuple=0;
	node->iss_TidPtr=0;
	node->fidx_state=CreateList;
	node->iss_mbufNum=MAX_LIBAIO;
	node->iss_remain_tid=0;
