...
/* DASOM : for multiple - local state for StartBufferIO and related functions */
static volatile BufferDesc **_InProgressBuf = NULL;
 ...
 * DASOM - 300715
 */
static volatile BufferDesc *BufferAlloc_Multiple(SMgrRelation smgr,
			char relpersistence,
			ForkNumber forkNum,
			BlockNumber blockNum,
			BufferAccessStrategy strategy,
			bool *foundPtr,
			int i);
static bool StartBufferIO_Multiple(volatile BufferDesc *buf, bool forInput, int i);
static void TerminateBufferIO_Multiple(volatile BufferDesc *buf, bool clear_dirty,
				  int set_flag_bits, int i);

/*
 * DASOM - 300715 - deleted
 * _InProgressBuf Allocation
 
 _InProgressBuf = (BufferDesc *)palloc(1 * sizeof(ItemPointerData));
*/ 
 ...
 *	FIXME:DASOM
 */
void ReadBufferMultiple2(Relation reln, Buffer *mbuf, int mbufNum, BlockNumber *blk)
{
	ReadBufferExtendedMultiple2(reln, MAIN_FORKNUM, mbuf, mbufNum, blk, RBM_NORMAL, NULL);
}
 ...
 */
void
ReadBufferExtendedMultiple2(Relation reln, ForkNumber forkNum, Buffer *mbuf, 
					int mbufNum, BlockNumber *blk,
				   	ReadBufferMode mode, BufferAccessStrategy strategy)
{
	bool		hit[mbufNum];
	int i;
	 ...
	 */
	pgstat_count_buffer_read(reln);
	ReadBufferMultiple_common2(reln->rd_smgr, reln->rd_rel->relpersistence, forkNum, blk,
...
/*
 * status : unstable
 * FIXME:DASOM 22nd Dec 2015
 * *blk is for keeping multiple blocknumbers 
 */
void
ReadBufferMultiple_common2(SMgrRelation smgr, char relpersistence, ForkNumber forkNum,
				  BlockNumber *blkNum, int mbufNum, Buffer *mbuf, ReadBufferMode mode,
				  BufferAccessStrategy strategy, bool *hit)
{
	...
	miss_count = 0;
	cb_list_index = 0;
	_InProgressBuf = (BufferDesc **)palloc(mbufNum * sizeof(BufferDesc)); 
	
	
	for(i = 0; i < mbufNum ; i++)
	{
		hit[i] = false;
		_InProgressBuf[i] = NULL;
	}
	...
	for(i = 0; i < mbufNum ; i++)
	{
		...
		else
		{
		 	...
		 	* not currently in memory.
		 	*/
			//fprintf(stderr, "BufferAlloc %d\n",i);
			bufHdr[i] = BufferAlloc_Multiple(smgr, relpersistence, forkNum, blkNum[i],
								 strategy, &found[i], i);
			if (found[i])
				pgBufferUsage.shared_blks_hit++;
			else
	...
	for(i = 0; i < mbufNum; i++)
	{
		...
		if (found[i])
		{
			...
			else
			{
				...
				} while (!StartBufferIO_Multiple(bufHdr[i], true, i));
			}
		} //the end of if(found)
		...
		if (isExtend[i])
		{
			...
			/* Set BM_VALID, terminate IO, and wake up any waiters. */
			TerminateBufferIO_Multiple(bufHdr[i], false, BM_VALID, i);
		}
		else
		{
			if (mode == RBM_ZERO)
...
/* Add functions for multiple IO
 * DASOM - 300715
 * BufferAlloc_Multiple -- subroutine for ReadBuffer.  Handles lookup of a shared
 *		buffer.  If no buffer exists already, selects a replacement
 *		victim and evicts the old page, but does NOT read in new page.
 *
 * "strategy" can be a buffer replacement strategy object, or NULL for
 ...
 */
static volatile BufferDesc *
BufferAlloc_Multiple(SMgrRelation smgr, char relpersistence, ForkNumber forkNum,
			BlockNumber blockNum,
			BufferAccessStrategy strategy,
			bool *foundPtr,
			int i)
	...
	if (buf_id >= 0)
	{
		...
		*foundPtr = TRUE;
		...
		if (!valid)
		{
			 ...
			 * StartBufferIO does it all.
			 */
			if (StartBufferIO_Multiple(buf, true, i))
			{
				/*
				 * If we get here, previous attempts to read the buffer must
				 * have failed ... but we shall bravely try again.
...
MarkBufferDirty(Buffer buffer)
{
	...
	if (!BufferIsValid(buffer)) {
		...
		/*DASOM : CHECK WHERE ERROR OCCURS*/
		fprintf(stderr, "Mark Buffer Dirty!\n");
...
PinBuffer(volatile BufferDesc *buf, BufferAccessStrategy strategy)
{
	...
	//fprintf(stderr, "PinBuffer - buf_id:%d\n",b);
...
#if DASOM_LOG
	/*DASOM - log*/
	fprintf(stderr, "PinBuffer - PrivateRefCount[%d]=%d , refcnt; %d\n", b,PrivateRefCount[b], buf->refcount);
#endif
	return result;
}
...
/*
		if(LWLockHeldByMe(buf->content_lock))
			elog(LOG, "failed Assertion buf_id: %d, lockid: %d", b,(int)buf->content_lock);
*/
...
#if DASOM_LOG
	/*DASOM - log*/
	fprintf(stderr, "Unpin Buffer - PrivateRefCount[%d]=%d , refcnt; %d\n", b,PrivateRefCount[b], buf->refcount);
#endif
}
...
ReleaseBuffer(Buffer buffer)
{
	volatile BufferDesc *bufHdr;
#if DASOM_LOG
	fprintf(stderr,"In ReleaseBuffer - %d\n", buffer-1);
#endif
	if (!BufferIsValid(buffer))
	{
#if DASOM_LOG
...
#if DASOM_LOG
	/*DASOM - log*/
	fprintf(stderr, "PrivateRefCount[%d]=%d , refcnt: %d\n", buffer-1,PrivateRefCount[buffer-1], bufHdr->refcount);
#endif
}
...
IncrBufferRefCount(Buffer buffer)
{
	...
	fprintf(stderr, "HERE!\n");
	if(BufferIsValid(buffer))
		fprintf(stderr, "Buffer %d is valid.\n", buffer-1);
	if(!BufferIsLocal(buffer))
		fprintf(stderr, "Buffer Private Ref Count[%d] : %d\n", buffer-1, PrivateRefCount[(buffer)-1]);
#endif
	Assert(BufferIsPinned(buffer));
	ResourceOwnerEnlargeBuffers(CurrentResourceOwner);
...
#if DASOM_LOG
	/*DASOM - log*/
	fprintf(stderr, "IncrBufferRefCount - PrivateRefCount[%d]=%d\n", buffer-1,PrivateRefCount[buffer-1]);
#endif
}
...
#if DASOM_LOG
		/*DASOM : CHECK WHERE ERROR OCCURS*/
		fprintf(stderr, "Mark Buffer Dirty HINT!\n");
#endif
		elog(ERROR, "bad buffer ID: %d", buffer);
	}
...
LockBuffer(Buffer buffer, int mode)
{
	...
	Assert(BufferIsValid(buffer));
#if DASOM_LOG
	fprintf(stderr, "LockBuffer buf_id: %d, mode: %d\n", buffer-1,mode);
#endif

	if (BufferIsLocal(buffer))
	{
		fprintf(stderr, "Local Buffer\n");
...
/*DASOM - Add functions for multiple it*/
/*
 * StartBufferIO_Multiple: begin I/O on this buffer
 *	(Assumptions)
 *	My process is executing no IO
 *	The buffer is Pinned
 *
 ...
 */
static bool
StartBufferIO_Multiple(volatile BufferDesc *buf, bool forInput, int i)
{
	Assert(!_InProgressBuf[i]);
...
/*DASOM - Add functions for multiple IO*/
/*
 * TerminateBufferIO_Multiple: release a buffer we were doing I/O on
 *	(Assumptions)
 *	My process is executing IO for the buffer
 *	BM_IO_IN_PROGRESS bit is set for the buffer
 *	We hold the buffer's io_in_progress lock
 ...
 */
static void
TerminateBufferIO_Multiple(volatile BufferDesc *buf, bool clear_dirty,
				  int set_flag_bits, int i)
{
	Assert(buf == _InProgressBuf[i]);
