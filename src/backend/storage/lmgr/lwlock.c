...
//	fprintf(stderr, "LWLockAcquire\n");
	...
	if (num_held_lwlocks >= MAX_SIMUL_LWLOCKS)
		elog(ERROR, "%d too many LWLocks taken",num_held_lwlocks);
...
/*
	elog(LOG, "held_lwlocks[%d] = lockid:(%d): excl %d shared %d head %p rOK %d",
		 (int) num_held_lwlocks-1,
		 (int) lockid,
		 (int) lock->exclusive, lock->shared, lock->head,
		 (int) lock->releaseOK);
*/
...
LWLockRelease(LWLockId lockid)
{
	...
	/*DASOM : log from PRINT_LWDEBUG*/
//	fprintf(stderr, "LWLockRelease\n");
/*	elog(LOG, "before - lockid(%d): excl %d shared %d head %p rOK %d",
		 (int) lockid,
		 (int) lock->exclusive, lock->shared, lock->head,
		 (int) lock->releaseOK);
*/
...
/*	
	elog(LOG, "after - lockid(%d): excl %d shared %d head %p rOK %d",
		 (int) lockid,
		 (int) lock->exclusive, lock->shared, lock->head,
		 (int) lock->releaseOK);
*/
