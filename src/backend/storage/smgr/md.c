...
/* FIXME : DASOM */
static struct iocb **ioq = NULL;
static io_context_t myctx;
static	struct io_event *ioevents;
static int wait_count = 0;
static int submit_count = 0;
#define LIBAIO_SUBMIT_COUNT	MAX_LIBAIO
...
md_read_start(int miss_count)
{
	...
	if(!ioq){
		...
		rc = io_queue_init(LIBAIO_SUBMIT_COUNT, &myctx);

		if(rc != 0)
			elog(LOG, "io_queue_init error!");

		ioq = (struct iocb **) malloc(sizeof(struct iocb *) * LIBAIO_SUBMIT_COUNT);

		for(i=0; i < LIBAIO_SUBMIT_COUNT ; i++)
			ioq[i] =  (struct iocb *) malloc(sizeof(struct iocb));

		ioevents  = (struct io_event*) malloc(sizeof(struct io_event)*LIBAIO_SUBMIT_COUNT);
	}
	else {
		/*cleansing ioq and ctx ??*/
	}
}
		...
		BlockNumber blocknum, char *buffer, int index)
{
	...
	io_prep_pread(ioq[index], GetFileFd(v->mdfd_vfd), buffer, BLCKSZ, seekpos);

#if 0 /*woonhak, remove this for test */
	io_set_callback(io, rd_done);
	ioq[index] = io;
#else
	/*submit current io request */
	int rc=io_submit(myctx, 1, &ioq[index]);      
	if(rc != 1)
	fprintf(stderr, "aio list io issue fail:%d\n", rc);
...
md_aio_read(int miss_count)
{
	...
	while(wait_count)
	{
#if 1 // DASOM io_missing count log
...
//	fprintf(stderr, "Submission size : %d\n", n_submit);
		
	for(i=0; i < n_submit; i += n_completed )
	{
		int remain_req = n_submit-i;
		int rc;
retry:
		/* Reaping */
		memset(ioevents, 0, sizeof(*ioevents)*remain_req);
		rc = io_getevents(myctx, remain_req, remain_req, ioevents, NULL);   

		/*now rc got the number of completed request*/
		if(rc > 0)
		{
			n_completed = rc;
			continue;
		...
		switch (rc) {
			...
			case 0:
				...
				fprintf(stderr, "Nothing done - reaping again for ssd write");
				goto retry;
		}
	}
#endif
