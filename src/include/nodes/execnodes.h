 ...
 * ----------------
 */
/* DASOM - ADD A NEW VARIABLE FOR TID ; NodeTag parentType */
typedef struct ExprContext
{
	NodeTag		type;

	NodeTag		parentType; /* DASOM - FOR TIDSCAN */
...
#if 0
	//FIXME: DASOM
//	ItemPointerData _tidList[MAX_LIBAIO]; // __IndexNext
 ...
 * ----------------
 */
/* DASOM - ADD A NEW VARIABLE TO CHECK ITS PARENT'S NODE TYPE ; FOR TIDSCAN	*/
typedef struct PlanState
{
	...
	NodeTag		parentType;		/* DASOM : CHECK ITS PARENT IS TIDSCAN OR NOT*/

#if 0 //FAIL
	int numtid; 				/* DASOM : 1st Oct*/
	ItemPointerData * _tidListptr;
	Buffer xs_mbuf[MAX_LIBAIO];
	HeapTupleData iss_htup[MAX_LIBAIO];
	int cur_tuple;
	int iss_TidPtr;
	bool multiple_read;
	int iss_mbufNum;
	int iss_remain_tid;
	FidxState fidx_state;
#endif
} PlanState;
...
#if 0 //Move them into IndexScan structure, 16th Oct 2015
	/*DASOM : for flash-idx ; is it okay to keep them here? - check please-14th,Oct*/
	Buffer xs_mbuf[MAX_LIBAIO];
	HeapTupleData iss_htup[MAX_LIBAIO];
	int cur_tuple;
	int iss_TidPtr;
...
typedef struct BitmapHeapScanState
{
	...
	TBMIterateResult tbmres[MAX_LIBAIO];
