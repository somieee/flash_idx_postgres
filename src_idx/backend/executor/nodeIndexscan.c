...
IndexNext_Multiple(IndexScanState *node)
{
	 ...
	 */
	pstate = node->ss.ps;
	tuple = pstate.iss_htup;
	buffer = pstate.xs_mbuf;
	
	if(pstate._tidListptr == NULL)
	{		
		pstate._tidListptr = (ItemPointerData *) 
								palloc(numberTuples * sizeof(ItemPointerData));
	
		numTids = &(pstate.numtid);
#if DASOM_LOG
		...
		if (pstate._tidListptr != NULL)
		{
			while ((tid = __index_getnext(scandesc, direction)) != NULL)
			{
				...
				if((*numTids) >= numberTuples)
				{
					numberTuples = numberTuples * 2;
					pstate._tidListptr = (ItemPointerData *)
						repalloc(pstate._tidListptr, numberTuples * sizeof(ItemPointerData));
				}

				pstate._tidListptr[(*numTids)]=*tid;
				(*numTids)++;
...
#endif
//		tuple=node->iss_htup;
		pstate.iss_remain_tid=*numTids;
		pstate.fidx_state=FetchTuples;
	}

	while (pstate.iss_remain_tid !=0)
	{
		tidList = pstate._tidListptr;
		switch((pstate.fidx_state))
		{
			case FetchTuples:
				/* Phase 3. Read tuples */
				if(pstate.iss_remain_tid < MAX_LIBAIO)
				{
					pstate.iss_mbufNum = pstate.iss_remain_tid;
				}
/*Instead of NULL for stats_relation, Pass scandesc->indexRelation*/
				if(heap_fetch_multiple(scandesc->heapRelation, scandesc->xs_snapshot, 
										tuple, false, scandesc->indexRelation, buffer, pstate.iss_mbufNum, 
										&tidList[pstate.iss_TidPtr]))
				{
#if DASOM_LOG
					fprintf(stderr, "LOG: Fetch tuples, %d\n", pstate.iss_mbufNum);
#endif
					pstate.fidx_state=ReturnTuples;
				}
				else 
					break;
			...
			case ReturnTuples:
				...
				while(pstate.cur_tuple < pstate.iss_mbufNum)
				{
					if(ItemPointerIsValid(&tidList[(pstate.iss_TidPtr)+(pstate.cur_tuple)]))
					{
						ExecStoreTuple(&tuple[pstate.cur_tuple],
									  slot,
									  buffer[pstate.cur_tuple],
									  false);
							
						ReleaseBuffer(buffer[pstate.cur_tuple]);
#if DASOM_LOG
...
#endif
				pstate.cur_tuple=0;
				pstate.iss_TidPtr+=pstate.iss_mbufNum;
				pstate.iss_remain_tid-=pstate.iss_mbufNum;
				pstate.fidx_state=FetchTuples;
