...
typedef struct f_smgr
{
	...
	/* FIXME:DASOM */
	void		(*smgr_read_start) (int miss_count);
	void		(*smgr_read_insert) (SMgrRelation reln, ForkNumber forkNum,
											BlockNumber blocknum, char *buffer, int index);
	void		(*smgr_aio_read) (int miss_count);
	void    	(*smgr_read_end) (int miss_count);
	/*end */								
...
static const f_smgr smgrsw[] = {
	...
	{mdinit, NULL, mdclose, mdcreate, mdexists, mdunlink, mdextend,
		mdprefetch, mdread, md_read_start, md_read_insert, md_aio_read, md_read_end, mdwrite, mdnblocks, mdtruncate, mdimmedsync,
...
/*
 * FIXME:DASOM referencing JIYOUNG
 */
void
smgr_read_start(SMgrRelation reln, int miss_count)
{
	(*(smgrsw[reln->smgr_which].smgr_read_start)) (miss_count);
}

void
smgr_read_insert(SMgrRelation reln, ForkNumber forknum, BlockNumber blocknum,
		char *buffer, int index)
{
	(*(smgrsw[reln->smgr_which].smgr_read_insert)) (reln, forknum, blocknum,
			buffer, index);
}

void
smgr_aio_read(SMgrRelation reln, int miss_count)
{
	(*(smgrsw[reln->smgr_which].smgr_aio_read)) (miss_count);
}

void
smgr_read_end(SMgrRelation reln, int miss_count)
{
	(*(smgrsw[reln->smgr_which].smgr_read_end)) (miss_count);
}
/* end */
